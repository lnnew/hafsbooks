    ...
    */
    ...
    passport.use(new FacebookStrategy(facebookCredentials,
        function (accessToken, refreshToken, profile, done) {
            ...
            pool.query('SELECT id FROM users WHERE email =$1',[email], (err, results) => {
              if(err) {
                return done(err)
              }
            ...
            if(results.rows[0]) {
              pool.query(
                'UPDATE users SET facebookid= $1 WHERE email = $2',
                [profile.id, email],
                (error, results) => {
                  if (error) {
                    throw error
              ...
              console.log("if",results.rows[0]);
               ...
               pool.query('INSERT INTO users (provider, id, email, displayName, facebookId) VALUES ($1, $2,$3,$4,$5)',
               ['facebook',shid, email, profile.displayName,profile.id ], (error, results) => {
                 if (error) {
                    throw error;
                 }
               })
